...
Runs the full pipeline against raw data that's already been downloaded:
    convert -> normalize -> clean -> deduplicate -> split

Raw downloading is kept as a separate step (`python -m src.data.download`)
on purpose, so acquiring raw data and generating the derived dataset stay
two distinct stages instead of getting tangled together.
...
def main(argv: list[str] | None = None) -> int:
    ...
    # step 1 — convert raw -> processed
    for source in enabled:
        print(f"\n=== convert:{source} " + "=" * 46)
        try:
            module = importlib.import_module(CONVERTER_MODULES[source])
            exit_code = module.convert(config)
        except Exception as exc:  # noqa: BLE001 - log it and keep going with the rest
