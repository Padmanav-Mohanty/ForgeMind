...
The CSV records that a merged PR fixed the linked issue (via GitHub link
keywords). It does NOT contain PR titles/bodies or diffs. The numeric
repo_id is resolved to owner/name through the GitHub API
(`GET /repositories/{id}`) once per unique id and cached deterministically.
PR descriptions are fetched from the raw dataset repository via
raw.githubusercontent.com when available.

The issue->PR relationship is always reported as an upstream fact ("this
PR was linked to and merged for this issue"); we never claim our own
verification of the fix.

Not every issue/PR pair is a good training example. The converter applies
explicit quality gates:
  - the issue must have a non-trivial title;
  - pairs without resolvable PR context are kept only when the issue body
    is substantive, and the assistant content then avoids describing the
    resolution;
  - everything must pass the shared cleaning thresholds.
...
# The GHPR README documents the dataset covers these 13 CNCF repositories
# (collected October 2020). repo_id values below were verified against the
# GitHub API (`GET /repositories/{id}` -> full_name) and against the
# per-repo instance counts in the GHPR README.
KNOWN_GHPR_REPOS = {
...
def _load_repo_id_map(unique_ids: list[str], token: str | None) -> dict[str, str]:
    ...
    Falls back to a small static table for the 13 CNCF repositories that
    the GHPR README documents, so offline runs still get correct slugs.
    API results (when available) take precedence and are cached in
    data/raw/ghpr/repo_id_map.json. Unresolvable ids stay absent (never
    guessed).
    """
    cached: dict[str, str] = {}
...
def _pr_summary_lines(pr_payload: dict[str, Any]) -> list[str]:
    """Human-readable PR description lines from a raw PR object."""
    lines: list[str] = []
...
def convert_pair(
    ...
    """Convert one (issue, PR) row into a ForgeMind example, or None."""
    ...
    # Quality gate without PR context: require a substantive issue body.
    ...
    metadata: dict[str, Any] = {
        ...
        # Group by the upstream issue so all examples derived from the same
        # issue (across linked PRs) stay in one split.
...
def _open_csv(path: Path):
    """Open a GHPR CSV with a raised field limit (some issue bodies are >128KB)."""
...
def _fetch_all_prs(
    ...
    raw.githubusercontent.com is a CDN (no meaningful rate limit), so a
    small thread pool keeps this phase to a couple of minutes. The
    returned dict covers every requested key; failures map to None.
    """
...
def convert(config: dict) -> int:
    """Convert the GHPR dataset. Returns process exit code."""
    ...
    token = get_github_token()  # optional; GitHub API is rate-limited without it
    ...
    # the same config always yields the same subset.
    rows.sort(
        key=lambda r: (
    ...
    # Fetch all unique PR objects up front (concurrently, deterministically
    # keyed), then build examples in row order.
    needed = sorted(
        {
