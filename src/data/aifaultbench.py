...
Important: AIFaultBench provides real-world AI software faults *and* the
evidence a reproducer observed, but it does NOT provide a known fix or a
curated root cause. The converter therefore builds:
...
No root cause or fix is ever fabricated: when the dataset does not contain
that information, the assistant content states what is known (observed
failure, evidence, reproduction command) and what is not (confirmed fix).
...
# Map AIFaultBench domains (as used in index.csv) onto the controlled task
# vocabulary. Domain describes the *software domain* of the fault; the task
# we train is fault analysis / debugging on that domain.
DOMAIN_TASKS = {
    "DL": "debugging",
    "LLM Infra": "debugging",
    "Agentic": "debugging",
...
def _parse_bug_report(text: str) -> dict[str, str]:
    ...
    if re.search(r"^TITLE\s*$", text, re.M):
        # Format 1 (may carry leading '# recovered-from-issue:' comment lines)
        ...
        return result
    # Format 2: structured header lines, body after 'Issue Body:'
...
def _extract_exception(log_text: str | None, max_lines: int = 40) -> str | None:
    """Extract the tail (exception + traceback) of a reproduction log."""
    if not log_text:
        return None
    lines = [line.rstrip() for line in log_text.splitlines() if line.strip()]
    if not lines:
        return None
    # Find the first line that looks like an exception header, searching
    # from the end so we keep the *final* traceback of the log.
    exc_start = None
...
def _language_for(library: str) -> str:
    # AIFaultBench reproduction packages are Python-based (repro.py,
    # requirements.txt). The upstream issue may concern other languages,
    # but the fault reproduction we have evidence for is Python.
    return "python"
...
def convert_bug(bug_dir: Path, index_row: dict[str, str], config: dict) -> dict[str, Any] | None:
    ...
    if not report["body"] and not report["title"]:
        return None  # nothing meaningful to build from
    ...
    # Failure summary: strictly from the issue and reproduction evidence.
    ...
    # Category hypothesis: allowed only as an explicitly hedged inference.
    assistant_parts.append("")
    ...
    # ------------------------------------------------------------------
    # Example 2: runtime failure diagnosis from the traceback (only when
    # the logs contain an exception; content stays grounded in the log).
    # ------------------------------------------------------------------
    settings = config.get("download", {}).get("aifaultbench", {})
...
def convert(config: dict) -> int:
    """Convert the full AIFaultBench raw dataset. Returns example count."""
