...
Raw data never gets modified after download — re-running is idempotent
and just skips directories that already exist. Delete a directory if you
...
# always downloaded — metadata, licensing, citation
...
# per-bug files the converter actually needs
...
# care about. Project source trees, build files, and bundled jars are
# excluded on purpose — they're huge and licensed upstream, not ours to ship.
...
def download_aifaultbench(config: dict, light: bool = False) -> Path:
    ...
    Resumable: an interrupted download is caught via a completion marker
    and just picks back up next run (snapshot_download already skips
...
def download_defects4j(config: dict) -> Path:
    """Sparse-clone just the Defects4J framework metadata and patches."""
    ...
    if marker.is_file():
        ...
        return dest
    if dest.is_dir() and any(dest.iterdir()):
        # looks like a partial checkout from an interrupted run — start clean
