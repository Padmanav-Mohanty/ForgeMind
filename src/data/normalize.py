"""Normalize processed examples without changing their meaning.
...
Phase 5 of the pipeline, applied between conversion and cleaning:
  ...
  - 3+ consecutive blank lines collapsed down to one
  - outer whitespace trimmed off message content
  - metadata keys with empty values dropped (schema rule)

Whitespace inside fenced code blocks is deliberately left alone, aside
from trailing spaces and CRLF endings — code formatting has to survive
this step intact.
...
def normalize_text(text: str) -> str:
    """Whitespace-normalize one message's content, keeping code blocks intact."""
    text = text.replace("\r\n", "\n").replace("\r", "\n")
    lines = text.split("\n")
    in_fence = False
    normalized: list[str] = []
    for line in lines:
        if line.lstrip().startswith("```"):
            ...
            continue
        # trailing whitespace always goes; leading whitespace stays inside
        # fences since indentation actually matters for code
        if in_fence:
            normalized.append(line.rstrip())
        else:
            normalized.append(line.rstrip() if line.strip() else "")
    text = "\n".join(normalized)
    text = re.sub(r"\n{4,}", "\n\n\n", text)  # only collapse really big gaps
    # strip leading blank lines and trailing whitespace, but never touch
    # leading indentation on the first content line — could be code
    text = re.sub(r"^(?:\s*\n)+", "", text)
    return text.rstrip()


def normalize_example(example: dict[str, Any]) -> dict[str, Any]:
    """Return a normalized copy of one example."""
